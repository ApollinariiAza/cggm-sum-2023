...
PSInput VSMain(float4 position : POSITION, float4 normal: NORMAL, float4 ambient : COLOR0, float4 diffuse : COLOR1,  float4 emissive : COLOR2, float2 texcoords : TEXCOORD)
{
    PSInput result;
...
PSInput VSShadowMap(float4 position : POSITION, float4 normal: NORMAL, float4 ambient : COLOR0, float4 diffuse : COLOR1,  float4 emissive : COLOR2, float2 texcoords : TEXCOORD)
{
    PSInput result;
...
float CalcUnshadowAmount(float3 world_position)
{
...
{
    float4 intensity = GetLambertianIntensity(input,
        light.position, light.color);
    return input.color * intensity * CalcUnshadowAmount(input.world_position);
}
float4 PSMain_texture(PSInput input) : SV_TARGET
{
    float4 intensity = GetLambertianIntensity(input,
        light.position, light.color);
    return g_texture.Sample(g_sampler, input.uv) * intensity * CalcUnshadowAmount(input.world_position);
}