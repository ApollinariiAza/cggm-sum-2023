...
namespace cg::renderer
{
			...
			const VB& vertex_a, const VB& vertex_b, const VB& vertex_c)
	{
		...
		diffuse = {vertex_b.diffuse_r, vertex_b.diffuse_g, vertex_b.diffuse_b};
		emissive = {vertex_c.emissive_r, vertex_c.emissive_g, vertex_c.emissive_b};
	}
			...
			float3 right, float3 up, size_t depth, size_t accumulation_num)
	{
		...
		{
			std::cout << "Tracking frame #" << frame_id + 1 << "\n";
