...
void cg::renderer::dx12_renderer::init()
{
	...
	shadow_light->set_height(static_cast<float>(settings->height));
	shadow_light->set_width(static_cast<float>(settings->width));
	shadow_light->set_position(float3{
			settings->camera_position[0],
...
void cg::renderer::dx12_renderer::update()
{
	...
	//{camera->get_position(), 1} - сточник света следует за камерой
	cb.light = {{shadow_light->get_position(), 1.f}, {0.70f, 0.60f, 0.50f, 1.f}};
...
ComPtr<IDXGIFactory4> cg::renderer::dx12_renderer::get_dxgi_factory()
{
	...
	THROW_IF_FAILED(CreateDXGIFactory2(dxgi_factory_flag,
											...
											ComPtr<ID3D12Resource>& intermediate_resource, D3D12_RESOURCE_STATES state_after, int row_pitch, int slice_pitch)
{
	...
	UpdateSubresources(command_list.Get(),
					   destination_resource.Get(), intermediate_resource.Get(), 0, 0, 1, &data);
...
{
	auto sampler_desc = get_sampler_descriptor();
	...
	for (size_t i = 0; i < model->get_index_buffers().size(); i++)
	{
		...
		create_resource_on_default_heap(
				vertex_buffers[i],
				vertex_buffer_size,
				vertex_buffer_name);
	...
	cbv_srv_heap.create_heap(
			...
			3 + static_cast<UINT>(model->get_per_shape_texture_files().size()),
	...
	null_srv_desc.Texture2D.ResourceMinLODClamp = 0.f;
	device->CreateShaderResourceView(nullptr, &null_srv_desc,
									 cbv_srv_heap.get_cpu_descriptor_handle(1));
	...
	device->CreateShaderResourceView(shadow_map.Get(), &shadow_srv_desc,
									 cbv_srv_heap.get_cpu_descriptor_handle(2));
	...
	for (size_t i = 0; i < model->get_per_shape_texture_files().size(); i++)
	{
		...
		std::string full_name = std::filesystem::absolute(
				model->get_per_shape_texture_files()[i]
										).string();
		...
		if (image == nullptr)
		{
			throw std::runtime_error("Can't load a texture");
		}
...
void cg::renderer::dx12_renderer::populate_command_list()
{
	...
	command_list->SetGraphicsRootDescriptorTable(
			2, cbv_srv_heap.get_gpu_descriptor_handle(1));
	...
	command_list->SetGraphicsRootDescriptorTable(
			2, cbv_srv_heap.get_gpu_descriptor_handle(2));
